## Header ----
    ...
    #
    # Notes: Phase 3.5 differs from previous phases and will be analyzed separately
    #        refer to notes in 00_phase3_5_prep_survey.R for details. 
    #        Phase 3.5 begins with week 46 (June 1 - 13, 2022)
    #
    # Needs rads >= v1.0.0
...
## Set up ----
    ...
    if(!exists("svy_msa") | !exists("svy_wa")){
      eval(parse(text = httr::content(httr::GET(
        url = "https://raw.githubusercontent.com/PHSKC-APDE/svy_pulse/main/00_phase3_5_prep_survey.R",
...
## Identify columns for calculations ----
    education_vars <- c("chldcare", paste0("chldimpct", 1:9), grep("teach_35", names(svy_wa), value = T))  
    ...
    vaccine_vars <- c("recvdvacc", "doses", "numdoses", 
                      "kidgetvac_5_11y", "kidgetvac_12_17y", "kidwhyno1", "kidwhyno2", "kidwhyno3", "kidwhyno4", "kidwhyno5", "kidwhyno6", 
          ...
          rbindlist(lapply(X = as.list(seq(1, nrow(housing.grid))),
                            FUN = function(X){
                              ...
                              tempDT <- rads::calc(ph.data = pooledN_svy_msa, # use data set for pooled weeks
                                                   ...
                                                   ifelse(paste0(housing.grid[1, myvars]) == "eviction", rentcur == 0, phase == "3.5") &
                                                     ifelse(paste0(housing.grid[1, myvars]) == "foreclosure", mortcur == 0, phase == "3.5") &
                                                     ifelse(paste0(housing.grid[1, myvars]) == "leave2mo", current == 0, phase == "3.5"),
    ...
    # Generic clean up function ----
        clean_up <- function(prefix = NULL){
          ...
          # Append to previous phase data if it exists ----    
              ...
              previousdt <- previousdt[phase != "3.5"] # drop data from phase 3.5 that will be replaced
...
## Clean up & save to Excel ----
    # education data ----
        ...
        writeDataTable(education_wb, sheet = "pulse", education[phase=="3.5"], colNames = TRUE, rowNames = FALSE)
        saveWorkbook(education_wb, file = paste0(outputdir, "education/pulse_phase3_5_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 3.5 results only
    ...
    # food security data ----
        ...
        writeDataTable(food_wb, sheet = "pulse", food[phase=="3.5"], colNames = TRUE, rowNames = FALSE)
        saveWorkbook(food_wb, file = paste0(outputdir, "food_security/pulse_phase3_5_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 3.5 results only
    ...
    # health / behavioral health data ----
        ...
        writeDataTable(health_wb, sheet = "pulse", health[phase=="3.5"], colNames = TRUE, rowNames = FALSE)
        saveWorkbook(health_wb, file = paste0(outputdir, "behavioral_health/pulse_phase3_5_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 3.5 results only
    ...
    # housing ----
        ...
        writeDataTable(housing_wb, sheet = "pulse", housing[phase=="3.5"], colNames = TRUE, rowNames = FALSE)
        saveWorkbook(housing_wb, file = paste0(outputdir, "housing/pulse_phase3_5_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 3.5 results only      
    ...
    # insured data ----
        ...
        writeDataTable(insured_wb, sheet = "pulse", insured[phase=="3.5"], colNames = TRUE, rowNames = FALSE)
        saveWorkbook(insured_wb, file = paste0(outputdir, "health_insurance/pulse_phase3_5_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 3.5 results only
    ...
    # vaccine data ----
        ...
        writeDataTable(vaccine_wb, sheet = "pulse", vaccine[phase=="3.5"], colNames = TRUE, rowNames = FALSE)
        saveWorkbook(vaccine_wb, file = paste0(outputdir, "vaccination/pulse_phase3_5_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 3.5 results only
