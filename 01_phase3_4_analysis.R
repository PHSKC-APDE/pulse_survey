## Header ----
    ...
    #
    # Notes: Phase 3.4 differs from previous phases and will be analyzed separately
    #        refer to notes in 00_phase3_4_prep_survey.R for details. 
    #        Phase 3.4 begins with week 43 (March 2 - 14, 2022)
    #
    # Needs rads >= v1.0.0
...
## Set up ----
    ...
    if(!exists("svy_msa3_4") | !exists("svy_wa3_4")){
      eval(parse(text = httr::content(httr::GET(
        url = "https://raw.githubusercontent.com/PHSKC-APDE/pulse_survey/main/00_phase3_4_prep_survey.R",
...
## Identify columns for calculations ----
    ...
    vaccine_vars <- c("recvdvacc", "doses", "definitevacc", "prevcovid", "whynorv1", "whynorv2", "whynorv3", "whynorv4", "whynorv5", 
                      ...
                      "numdoses", "brand", 
                      "kiddoses_5_11y", "kiddoses_12_17y",  "kidgetvac_5_11y", "kidgetvac_12_17y", "kidwhyno1", "kidwhyno2", "kidwhyno3", "kidwhyno4", "kidwhyno5", "kidwhyno6", 
                      "kidwhyno7", "kidwhyno8", "kidwhyno9", "kidwhyno10", "kidwhyno11", "kidwhyno12", "kidwhyno13", "kidwhyno14")
...
## Use rads to perform calculations ----
    ...
    # education: by week ----
        education_weeks_wa <- rbind(
          calc(ph.data = svy_wa3_4, # use data set for individual weeks (NOT POOLED weights)
        ...
        education_weeks_msa <- rbind(
          calc(ph.data = svy_msa3_4, # use data set for individual weeks (NOT POOLED weights)
        ...
        education_msa_combo <- rbind(
          ...
          rbindlist(lapply(X = as.list(seq(1, nrow(edu.cat.grid))), 
                           FUN = function(X){
                             ...
                             tempDT <- rads::calc(ph.data = pooledN_svy_msa3_4, # use data set for pooled weeks
        ...
        education_wa_combo <- rbind(
          ...
          rbindlist(lapply(X = as.list(seq(1, nrow(edu.cat.grid))), 
                           FUN = function(X){
                             ...
                             tempDT <- rads::calc(ph.data = pooledN_svy_wa3_4, # use data set for pooled weeks
                                                  what = paste0(edu.cat.grid[X, myvars]),
                                                  by = paste0(edu.cat.grid[X, byvars]),
                                                  time_var = "week",
                                                  metrics = c("mean", "rse", "denominator", "numerator"),
    ...
    # Health: by week ----
        health_weeks_wa <- calc(ph.data = svy_wa3_4, # use data set for individual weeks (NOT POOLED weights)
                           what = health_vars,
                           by = c("week", "phase"),
                           metrics = c("mean", "rse", "denominator", "numerator"),
                           proportion = T,
        ...
        health_weeks_msa <- calc(ph.data = svy_msa3_4, # use data set for individual weeks (NOT POOLED weights)
                            what = health_vars,
                            by = c("week", "phase"),
                            metrics = c("mean", "rse", "denominator", "numerator"),
                            proportion = T,
    ...
    # Housing: by week ----
        housing_weeks_wa <- calc(ph.data = svy_wa3_4, # use data set for individual weeks (NOT POOLED weights)
                           what = housing_vars,
                           by = c("week", "phase"),
                           metrics = c("mean", "rse", "denominator", "numerator"),
                           proportion = T,
        ...
        housing_weeks_msa <- calc(ph.data = svy_msa3_4, # use data set for individual weeks (NOT POOLED weights)
                            what = housing_vars,
                            by = c("week", "phase"),
                            metrics = c("mean", "rse", "denominator", "numerator"),
                            proportion = T,
                                              ...
                                              FUN = function(X){
                                                ...
                                                tempDT <- rads::calc(ph.data = pooledN_svy_msa3_4, # use data set for pooled weeks
                                                                     what = paste0(housing.grid[X, myvars]),
                                                                       ifelse(paste0(housing.grid[1, myvars]) == "eviction", rentcur == 0, phase == "3.4") &
                                                                       ifelse(paste0(housing.grid[1, myvars]) == "foreclosure", mortcur == 0, phase == "3.4") &
                                                                       ifelse(paste0(housing.grid[1, myvars]) == "leave2mo", current == 0, phase == "3.4"),
    ...
    # Insured: by week ----
        insured_weeks_wa <- calc(ph.data = svy_wa3_4, # use data set for individual weeks (NOT POOLED weights)
                                what = insured_vars,
                                by = c("week", "phase"),
                                metrics = c("mean", "rse", "denominator", "numerator"),
                                proportion = T,
        ...
        insured_weeks_msa <- calc(ph.data = svy_msa3_4, # use data set for individual weeks (NOT POOLED weights)
                                 what = insured_vars,
                                 by = c("week", "phase"),
                                 metrics = c("mean", "rse", "denominator", "numerator"),
                                 proportion = T,
    ...
    # Insured: pooled demographics ----
        ...
        insured_msa_combo_age4 <- rads::calc(ph.data = pooledN_svy_msa3_4, # use dataset for pooled weeks 
        ...
        insured_wa_combo_age4 <- rads::calc(ph.data = pooledN_svy_wa3_4, # use dataset for pooled weeks 
                                             what = c("insured", "uninsured"), 
                                             age4 != "65+",
                                             by = "age4", 
                                             time_var = "week", 
    ...
    # Vaccine: by week ----
        vaccine_weeks_wa <- calc(ph.data = svy_wa3_4, # use data set for individual weeks (NOT POOLED weights)
                                 what = vaccine_vars,
                                 by = c("week", "phase"),
                                 metrics = c("mean", "rse", "denominator", "numerator"),
                                 proportion = T,
        ...
        vaccine_weeks_msa <- calc(ph.data = svy_msa3_4, # use data set for individual weeks (NOT POOLED weights)
                                  what = vaccine_vars,
                                  by = c("week", "phase"),
                                  metrics = c("mean", "rse", "denominator", "numerator"),
                                  proportion = T,
    ...
    # Generic clean up function ----
        clean_up <- function(prefix = NULL){
          ...
          # Estimates by demographics ----
              ...
              catcombo[, week := paste0("43-", max(na.omit(suppressWarnings(as.integer(wkcombo$week)))))]
          ...
          # Append to previous phase data if it exists ----    
              ...
              previousdt <- previousdt[phase != "3.4"] # drop data from phase 3.4 that will be replaced
...
## Clean up & save to Excel ----
    # education data ----
        ...
        writeDataTable(education_wb, sheet = "pulse", education[phase=="3.4"], colNames = TRUE, rowNames = FALSE)
        saveWorkbook(education_wb, file = paste0(paste0(outputdir, "education/pulse_phase3_4_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 3.4 results only
    ...
    # food security data ----
        ...
        writeDataTable(food_wb, sheet = "pulse", food[phase=="3.4"], colNames = TRUE, rowNames = FALSE)
        saveWorkbook(food_wb, file = paste0(paste0(outputdir, "food_security/pulse_phase3_4_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 3.4 results only
    ...
    # health / behavioral health data ----
        ...
        writeDataTable(health_wb, sheet = "pulse", health[phase=="3.4"], colNames = TRUE, rowNames = FALSE)
        saveWorkbook(health_wb, file = paste0(paste0(outputdir, "behavioral_health/pulse_phase3_4_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 3.4 results only
    ...
    # housing ----
        ...
        writeDataTable(housing_wb, sheet = "pulse", housing[phase=="3.4"], colNames = TRUE, rowNames = FALSE)
        saveWorkbook(housing_wb, file = paste0(paste0(outputdir, "housing/pulse_phase3_4_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 3.4 results only      
    ...
    # insured data ----
        ...
        writeDataTable(insured_wb, sheet = "pulse", insured[phase=="3.4"], colNames = TRUE, rowNames = FALSE)
        saveWorkbook(insured_wb, file = paste0(paste0(outputdir, "health_insurance/pulse_phase3_4_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 3.4 results only
    ...
    # vaccine data ----
        ...
        writeDataTable(vaccine_wb, sheet = "pulse", vaccine[phase=="3.4"], colNames = TRUE, rowNames = FALSE)
        saveWorkbook(vaccine_wb, file = paste0(paste0(outputdir, "vaccination/pulse_phase3_4_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 3.4 results only
