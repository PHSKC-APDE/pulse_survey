## Header ----
    ...
    # Date: May 26, 2022
    # R version: 4.1.2
    # Purpose: append and prep all individual PULSE 3.2 survey files for analysis
    #
    ...
    #
    #
    # Notes: Phase 3_2 differs from Phase 2, 3, & 3.1 and it is also a different stage of the pandemic, so it will be analyzed separately
    # 
...
## Prepare data ----
    ...
    # create variables ----
        ...
        # Vaccine ----      
          ...
          # QV 2
          dt[, dosesrv := factor(dosesrv, levels = 1:3, labels = c("Yes, received all required doses", "Yes, plan to receive all required doses", "No, don't plan to receive all required doses"))]
          
          dt[dosesrv == "No, don't plan to receive all required doses", doses := 0]
          dt[dosesrv != "No, don't plan to receive all required doses", doses := 1]
          dt[, doses := factor(doses, levels = 0:1, labels = c("No", "Yes"))]
          ...
          for(col in grep("whynorv[0-9]", names(dt), value = T)){
            dt[(dosesrv == "No, don't plan to receive all required doses" | getvacrv %in% 2:5) & is.na(get(col)), paste0(col) := 0]
