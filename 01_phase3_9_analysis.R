## Header ----
    ...
    #
    # Notes: Phase 3.9 differs from previous phases and will be analyzed separately
    #        refer to notes in 00_phase3_8=9_prep_survey.R for details. 
    #        Phase 3.9 begins with week 58 (June 7-19, 2023)
    #
    # Needs rads >= v1.0.0
...
## Set up ----
    ...
    # set constants ----
    phase.number <- "3.9"
    phase_number <- "3_9" # have to give this value a second time because creating the analysis dataset
    first.week <- 58 # identify the first week of the current phase
...
## Identify columns for calculations ----
    #education_vars <- c(grep("teach_35", names(svy_wa), value = T))  # ended with phase 3.9
    ...
    housing_vars <- c("tenure2", "rent_change.gt250", "rent_change", "rentcur", "mortcur", "current", "notcurrent", "eviction", "foreclosure", "leave2mo", "leave2mo_alt", 
                      "movewhy1", "movewhy2", "movewhy3", "movewhy4", "movewhy5", "movewhy6", "movewhy7", "movewhy8", "moved")
    # housing_vars_noncat <- c("rent_monthly") # ended with Phase 3.9
    
    insured_vars <- c("insurance", "uninsured", "insured", "insured_employer", "insured_exchange", "insured_mcare", "insured_military", 
                      "medicaidv2", "medicaid1", "medicaid3", "medicaid_no")
...
## Use rads to perform calculations ----
    ...
    # Housing: pooled demographics ----
        ...
        # housing.grid.noncat <- setDT(expand.grid(myvars = housing_vars_noncat, byvars = mybyvars)) # ended Phase 3.9
    ...
    # Vaccine: by week (no longer as of Phase 3.8) ----
      
    # Vaccine: pooled demographics (no longer as of Phase 3.8) ----
...
## Clean up & save to Excel ----
    ...
    # vaccine data (no longer as of Phase 3.8) ----
