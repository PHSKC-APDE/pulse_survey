## Header ----
    ...
    #
    # Notes: Phase 3.3 differs from previous phases and will be analyzed separately
    #        refer to notes in 00_phase3_3_prep_survey.R for details. 
    #        Phase 3.3 begins with week 40 (December 1 - 13, 2021)
    #
    # Needs rads >= v1.0.0
...
## Identify columns for calculations ----
    ...
    vaccine_vars <- c("recvdvacc", "doses", "definitevacc", "prevcovid", "whynorv1", "whynorv2", "whynorv3", "whynorv4", "whynorv5", 
                      ...
                      "numdoses", "brand", 
                      "kiddoses", "kidgetvac", "kidwhyno1", "kidwhyno2", "kidwhyno3", "kidwhyno4", "kidwhyno5", "kidwhyno6", 
                                              ...
                                              FUN = function(X){
                                                ...
                                                tempDT <- rads::calc(ph.data = pooledN_svy_msa, # use data set for pooled weeks
                                                                     what = paste0(housing.grid[X, myvars]),
                                                                       ...
                                                                       ifelse(paste0(housing.grid[1, myvars]) == "leave2mo", current == 0, phase == "3.3"),
    ...
    # Generic clean up function ----
        clean_up <- function(prefix = NULL){
          ...
          # Estimates by demographics ----
              ...
              catcombo[, week := paste0("40-", max(na.omit(suppressWarnings(as.integer(wkcombo$week)))))]
