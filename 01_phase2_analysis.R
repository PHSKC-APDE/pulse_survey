...
## set up ----
    ...
    if(!exists("svy_msa1") | !exists("svy_wa1")){
      eval(parse(text = httr::content(httr::GET(
        url = "https://raw.githubusercontent.com/PHSKC-APDE/pulse_survey/main/00_phase2_prep_survey.R",
...
## Use rads to perform calculations ----
    ...
    # education: by week ----
        education_weeks_wa <- rbind(
          calc(ph.data = svy_wa1, # use data set for individual weeks (NOT POOLED weights)
        ...
        education_weeks_msa <- rbind(
          calc(ph.data = svy_msa1, # use data set for individual weeks (NOT POOLED weights)
    ...
    # education: pooled demographics ----
        education_msa_combo <- rbind(
          ...
          rbindlist(lapply(X = as.list(mybyvars),
                           FUN = function(X){
                             tempDT <- rads::calc(ph.data = pooledN_svy_msa1 %>% filter(!is.na(get(X))), # use data set for pooled weeks
                                                  what = education_vars,
                                                  week >= 1,
                                                  by = X,
                                                  time_var = "week",
        ...
        education_wa_combo <- rbind(
          ...
          rbindlist(lapply(X = as.list(mybyvars),
                           FUN = function(X){
                             tempDT <- rads::calc(ph.data = pooledN_svy_wa1 %>% filter(!is.na(get(X))), # use data set for pooled weeks
                                                  what = education_vars,
                                                  week >= 1,
                                                  by = X,
                                                  time_var = "week",
    ...
    # Food security: pooled demographics ----
        ...
        food_msa_combo <- rbindlist(lapply(X = as.list(seq(1, nrow(mycombos), 1)),
                                             FUN = function(X){
                                               tempDT <- rads::calc(ph.data = pooledN_svy_msa1, # use data set for pooled weeks
        ...
        food_wa_combo <- rbindlist(lapply(X = as.list(seq(1, nrow(mycombos), 1)),
                                           FUN = function(X){
                                             tempDT <- rads::calc(ph.data = pooledN_svy_wa1, # use data set for pooled weeks
                                                                  what = c("curfoodsuf", grep("where", names(dt), value = T), "freefood"),
                                                                  week %in% paste0(mycombos[X, ]$weekstart):paste0(mycombos[X, ]$weekend),
                                                                  by = paste0(mycombos[X, ]$myvars),
                                                                  time_var = "week",
    ...
    # Health: by week ----
        health_weeks_wa <- calc(ph.data = svy_wa1, # use data set for individual weeks (NOT POOLED weights)
                           what = health_vars,
                           by = c("week", "phase"),
                           metrics = c("mean", "rse", "denominator", "numerator"),
                           proportion = T,
        ...
        health_weeks_msa <- calc(ph.data = svy_msa1, # use data set for individual weeks (NOT POOLED weights)
                            what = health_vars,
                            by = c("week", "phase"),
                            metrics = c("mean", "rse", "denominator", "numerator"),
                            proportion = T,
    ...
    # Housing: by week ----
        housing_weeks_wa <- calc(ph.data = svy_wa1, # use data set for individual weeks (NOT POOLED weights)
                           what = housing_vars,
                           by = c("week", "phase"),
                           metrics = c("mean", "rse", "denominator", "numerator"),
                           proportion = T,
        ...
        housing_weeks_msa <- calc(ph.data = svy_msa1, # use data set for individual weeks (NOT POOLED weights)
                            what = housing_vars,
                            by = c("week", "phase"),
                            metrics = c("mean", "rse", "denominator", "numerator"),
                            proportion = T,
    ...
    # Housing: pooled demographics ----
        housing_msa_combo <- rbindlist(lapply(X = as.list(mybyvars),
                                           FUN = function(X){
                                             tempDT <- rbind(
                                               rads::calc(ph.data = pooledN_svy_msa1, # use data set for pooled weeks
        ...
        housing_wa_combo <- rbindlist(lapply(X = as.list(mybyvars),
                                            FUN = function(X){
                                              tempDT <- rbind(
                                                rads::calc(ph.data = pooledN_svy_wa1, # use data set for pooled weeks
                                                           what = setdiff(housing_vars, c("eviction", "foreclosure", "leave2mo")),
                                                           week >= 1,
                                                           by = X,
                                                           time_var = "week",
    ...
    # Insured: by week ----
        insured_weeks_wa <- calc(ph.data = svy_wa1, # use data set for individual weeks (NOT POOLED weights)
                                what = insured_vars,
                                by = c("week", "phase"),
                                metrics = c("mean", "rse", "denominator", "numerator"),
                                proportion = T,
        ...
        insured_weeks_msa <- calc(ph.data = svy_msa1, # use data set for individual weeks (NOT POOLED weights)
                                 what = insured_vars,
                                 by = c("week", "phase"),
                                 metrics = c("mean", "rse", "denominator", "numerator"),
                                 proportion = T,
    ...
    # Insured: pooled demographics ----
        ...
        insured_msa_combo_age4 <- rads::calc(ph.data = pooledN_svy_msa1, # use dataset for pooled weeks 
        ...
        insured_wa_combo_age4 <- rads::calc(ph.data = pooledN_svy_wa1, # use dataset for pooled weeks 
                                             what = c("insured", "uninsured"), 
                                             week >= 1  & age4 != "65+",
                                             by = "age4", 
                                             time_var = "week", 
        ...
        insured_msa_combo <- rbindlist(lapply(X = as.list( seq(1, nrow(grid.insured)) ), 
                                              FUN = function(X){print(paste("Processing row", X, "of", nrow(grid.insured), "in 'grid.insured'(", grid.insured[X, 1], ":", grid.insured[X, 2], ")"))
                                                tempDT <- rads::calc(ph.data = pooledN_svy_msa1, # use dataset for pooled weeks 
        ...
        insured_wa_combo <- rbindlist(lapply(X = as.list( seq(1, nrow(grid.insured)) ), 
                                              FUN = function(X){print(paste("Processing row", X, "of", nrow(grid.insured), "in 'grid.insured'(", grid.insured[X, 1], ":", grid.insured[X, 2], ")"))
                                                tempDT <- rads::calc(ph.data = pooledN_svy_wa1, # use dataset for pooled weeks 
                                                                     what = paste0(grid.insured[X, 1]), 
                                                                     week >= 1 & !is.na(get(paste0(grid.insured[X, 2]))), # skip when byvar is NA
                                                                     by = paste0(grid.insured[X, 2]), 
                                                                     time_var = "week", 
    ...
    # Vaccine: by week ----
        vaccine_weeks_wa <- calc(ph.data = svy_wa1, # use data set for individual weeks (NOT POOLED weights)
                                 what = vaccine_vars,
                                 week >= 22,
                                 by = c("week", "phase"),
                                 metrics = c("mean", "rse", "denominator", "numerator"),
        ...
        vaccine_weeks_msa <- calc(ph.data = svy_msa1, # use data set for individual weeks (NOT POOLED weights)
                                  what = vaccine_vars,
                                  week >= 22,
                                  by = c("week", "phase"),
                                  metrics = c("mean", "rse", "denominator", "numerator"),
    ...
    # Generic clean up function ----
        clean_up <- function(prefix = NULL){
          ...
          # Append to phase 1 data if it exists ----    
              ...
              if(prefix == "insured"){phase1dt <- setDT(openxlsx::read.xlsx(paste0(paste0(outputdir, "health_insurance/pulse_results.xlsx")), sheet = 'pulse'))}
...
## Clean up & save to Excel ----
    # education data ----
        ...
        saveWorkbook(education_wb, file = paste0(paste0(outputdir, "education/pulse_phase2_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 2 results only
    ...
    # food security data ----
        ...
        saveWorkbook(food_wb, file = paste0(paste0(outputdir, "food_security/pulse_phase2_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 2 results only
    ...
    # health / behavioral health data ----
        ...
        saveWorkbook(health_wb, file = paste0(paste0(outputdir, "behavioral_health/pulse_phase2_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 2 results only
    ...
    # housing ----
        ...
        saveWorkbook(housing_wb, file = paste0(paste0(outputdir, "housing/pulse_phase2_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 2 results only      
    ...
    # insured data ----
        ...
        saveWorkbook(insured_wb, file = paste0(paste0(outputdir, "health_insurance/pulse_phase2_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 2 results only
    ...
    # vaccine data ----
        ...
        saveWorkbook(vaccine_wb, file = paste0(paste0(outputdir, "vaccination/pulse_phase2_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 2 results only
