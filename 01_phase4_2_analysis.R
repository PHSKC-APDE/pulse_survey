## Header ----
    ...
    #
    # Notes: Phase 4_2 is similar to Phase 4_0, except for support4 >> support4_rv
    #
    # Needs rads >= v1.0.0
    #
...
## Set up ----
    ...
    # set constants ----
      phase.number <- "4_2"
      phase_number <- "4_2" # have to give this value a second time because creating the analysis dataset
      first.cycle <- 8 # identify the first cycle of the current phase
    ...
    # load prepped data if necessary ----
      if(!exists("svy_msa") | !exists("svy_wa")){
        source(here::here(paste0("00_phase", phase.number, "_prep_survey.R")))
      }
...
## Identify columns for calculations ----
    #education_vars <- c(grep("teach_35", names(svy_wa), value = T))  # ended with phase 3.10
    ...
    food_vars <- c("childfood.bin", "curfoodsuf", "freefood" ,
                   ...
                   "fdbenefitsnap", "fdbenefitwic", "fdbenefitnone")    
    ...
    health_vars <- c("down", "anxious", "worry", "interest", 
                     ...
                     "social1", "social2", "support1", "support2", "support3", "support4_rv")

    housing_vars <- c("tenure2", "rent_change.gt250", "rent_change", 
                      "rentcur", "mortcur", "current", "notcurrent", 
                      "eviction", "foreclosure", "leave2mo", "leave2mo_alt", 
                      "movewhy1", "movewhy2", "movewhy3", "movewhy4", "movewhy5", 
                      "movewhy6", "movewhy7", "movewhy8", "moved")
    # housing_vars_noncat <- c("rent_monthly") # ended with Phase 3.10
    
    insured_vars <- c("insurance", "uninsured", "insured", "insured_employer", 
                      "insured_exchange", "insured_mcare", "insured_military")

    vaccine_vars <- c() # no longer process vaccine related vars as of the start of Phase 3.8 (week 49)
...
## Make sure all the variables identified as being present actually exist ----
    varvectors <- grep('vars', ls(), value = T)
    missingVars <- data.table()
    for(varvector in varvectors){
      for(testvar in get(varvector)){
        if(!testvar %in% names(svy_wa)){missingVars <- rbind(missingVars <- rbind(missingVars, data.table(category = varvector, variable = testvar)))}
      }
    }
    
    missingVartable <- vartable[variable %in% missingVars$variable & 
                                  !grepl('households with children$', description, ignore.case = T)] # don't worry about these, because generated on the fly with calc
    
    if(nrow(missingVartable) > 0){
      print(missingVartable)
      stop(paste0("\n\U0001f47f The above shows the expected variables which are missing in your data.", 
                  "Update the prep code and or the code above. Also review the vartable."))
    }
...
## Use rads to perform calculations ----
    ...
    # education: by cycle ----
        education_cycles_wa <- rbind(
          calc(ph.data = svy_wa, # use data set for individual cycles (NOT POOLED weights)
        ...
        education_cycles_msa <- rbind(
          calc(ph.data = svy_msa, # use data set for individual cycles (NOT POOLED weights)
    ...
    # education: by demographics ----
        edu.cont.grid <- setDT(expand.grid(myvars = education_vars_noncat, byvars = setdiff(mybyvars, "alone")))
    
        education_msa_combo <- rbindlist(lapply(X = as.list(as.list(seq(1, nrow(edu.cont.grid)))),
                                                FUN = function(X){
                                                  tempDT <- rads::calc(ph.data = svy_msa, # use data set for pooled cycles
        ...
        education_wa_combo <- rbindlist(lapply(X = as.list(as.list(seq(1, nrow(edu.cont.grid)))),
                                                FUN = function(X){
                                                  tempDT <- rads::calc(ph.data = svy_wa, # use data set for pooled cycles
                                                                       what = paste0(edu.cont.grid[X, myvars]),
                                                                       by = paste0(edu.cont.grid[X, byvars]),
                                                                       time_var = "cycle",
                                                                       metrics = c("mean", "rse", "denominator", "numerator"),
    ...
    # Food security: by cycle ----
        food_cycles_wa <- rbind(
                          calc(ph.data = svy_wa, # use data set for individual cycles (NOT POOLED weights)
                                what = c("curfoodsuf", grep("foodwhynot|childfood.bin|fdbenefit", names(dt), value = T), "freefood"), # insufficient food, couldn't afford, afraid, stores didn't have what was wanted, free food
                                by = c("cycle", "phase"),
                                metrics = c("mean", "rse", "denominator", "numerator"),
                                proportion = T,
                                ci = 0.90)[, hh:= "All"], 
    ...
    # Food security: by demographics ---- 
        ...
        mycombos <- setDT(expand.grid(myvars = c(setdiff(mybyvars, "gender"), "children"), cyclestart = min(food_cycles_wa$cycle) , cycleend = max(food_cycles_wa$cycle)))
        mycombos <- mycombos[(cyclestart==min(food_cycles_wa$cycle) & cycleend==max(food_cycles_wa$cycle))]
        
        food_msa_combo <- rbindlist(lapply(X = as.list(seq(1, nrow(mycombos), 1)),
                                           FUN = function(X){
                                             # message(paste0("Food MSA combo: row ", X, " of ", nrow(mycombos)))
                                             tempDT <- rads::calc(ph.data = svy_msa, 
                                                                  what = c("curfoodsuf", grep("where|foodwhy|fdbenefit", names(dt), value = T), "freefood"),
                                                                  cycle %in% paste0(mycombos[X, cyclestart]):paste0(mycombos[X, cycleend]),
                                             ...
                                             tempDT[, geo := "MSA"]
                                             tempDT <- tempDT[!is.na(group)]
                                             tempDT <- tempDT[, cycle := paste0(mycombos[X, ]$cyclestart, "-", mycombos[X, ]$cycleend)]
                                           }), use.names = T )
        
        food_wa_combo <- rbindlist(lapply(X = as.list(seq(1, nrow(mycombos), 1)),
                                          FUN = function(X){
                                            # message(paste0("Food MSA combo: row ", X, " of ", nrow(mycombos)))
                                            tempDT <- rads::calc(ph.data = svy_wa, 
                                                                 what = c("curfoodsuf", grep("where|foodwhy|fdbenefit", names(dt), value = T), "freefood"),
                                                                 cycle %in% paste0(mycombos[X, cyclestart]):paste0(mycombos[X, cycleend]),
                                                                 by = paste0(mycombos[X, myvars]),
                                                                 time_var = "cycle",
    ...
    # Health: by cycle ----
        health_cycles_wa <- calc(ph.data = svy_wa, # use data set for individual cycles (NOT POOLED weights)
                           what = health_vars,
                           by = c("cycle", "phase"),
                           metrics = c("mean", "rse", "denominator", "numerator"),
                           proportion = F, 
        ...
        health_cycles_msa <- calc(ph.data = svy_msa, # use data set for individual cycles (NOT POOLED weights)
                            what = health_vars,
                            by = c("cycle", "phase"),
                            metrics = c("mean", "rse", "denominator", "numerator"),
                            proportion = T,
    ...
    # Insured: by cycle ----
        insured_cycles_wa <- calc(ph.data = svy_wa, # use data set for individual cycles (NOT POOLED weights)
                                what = insured_vars,
                                by = c("cycle", "phase"),
                                metrics = c("mean", "rse", "denominator", "numerator"),
                                proportion = T,
        ...
        insured_cycles_msa <- calc(ph.data = svy_msa, # use data set for individual cycles (NOT POOLED weights)
                                 what = insured_vars,
                                 by = c("cycle", "phase"),
                                 metrics = c("mean", "rse", "denominator", "numerator"),
                                 proportion = T,
    ...
    # Insured: by demographics ----
        # Insured doesn't apply to 65+ so need calc age bins separately
        insured_msa_combo_age4 <- rads::calc(ph.data = svy_msa, # use dataset for pooled cycles 
        ...
        insured_wa_combo_age4 <- rads::calc(ph.data = svy_wa, # use dataset for pooled cycles 
                                            what = c("insured", "uninsured"), 
                                            age4 != "65+",
                                            by = "age4", 
                                            time_var = "cycle", 
...
## Clean up functions ----
    # Generic clean up function ----
        clean_up <- function(prefix = NULL){
          # Estimates by cycle ----
              wkcombo <- rbind(get(paste0(prefix, '_cycles_wa'))[, geo := "WA"], 
                               get(paste0(prefix, '_cycles_msa'))[, geo := "MSA"])
              wkcombo[, phase := NULL]
              wkcombo[, category := "cycle"]
              wkcombo[, group := cycle]
          ...
          # Estimates by demographics ----
              ...
              catcombo[, cycle := paste0(first.cycle, "-", max(na.omit(suppressWarnings(as.integer(wkcombo$cycle)))))]
              catcombo[category == "phase", group := "Total"]
              catcombo[category == "phase", category := "all"]    
              
          # Combine cycle and stratified estimates ----
              ...
              if(!prefix %in% c("food")){
                ...
                combo <- combo[, .(cycle, source = "Pulse", geo, variable, level, category, group, percent = mean, 
          ...
          # Format identical to Lin Song's output ----
              ...
              if(!prefix %in% c("food", "education", "housing", "insured", "vaccine", "health")){
                combo[, level := NULL]
              } 
              
          # Add label for phase/cycle ----
          ...
          # Rename 'cycle' column to 'week' in order to merge estimates from previous Phases
              setnames(combo, 'cycle', 'week')
          ...
          # Append to previous phase data if it exists ----    
              ...
              previousdt <- previousdt[phase != phase.number] # drop data from phase 3.8 that will be replaced
...
## Clean up & save to Excel ----
    # education data ----
        ...
        education_vars <- c(education_vars_noncat) # need to create a complete list of all education variables
    ...
    # food security data ----
        ...
        saveWorkbook(food_wb, file = paste0(outputdir, "food_security/pulse_phase", phase_number, "_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 3.8 results only
    ...
    # health / behavioral health data ----
        ...
        saveWorkbook(health_wb, file = paste0(outputdir, "behavioral_health/pulse_phase", phase_number, "_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 3.8 results only
        
    # housing ----
        # housing_vars <- c(housing_vars, housing_vars_noncat) # need to create a complete list of housing variables
        housing_vars <- c(housing_vars) # need to create a complete list of housing variables
        ...
        saveWorkbook(housing_wb, file = paste0(outputdir, "housing/pulse_phase", phase_number, "_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 3.8 results only      
    ...
    # insured data ----
        ...
        saveWorkbook(insured_wb, file = paste0(outputdir, "health_insurance/pulse_phase", phase_number, "_results_", gsub("-", "_", Sys.Date()), ".xlsx"), overwrite = TRUE) # has phase 3.8 results only
