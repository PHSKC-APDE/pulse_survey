## Header ----
    ...
    # Date: May 26, 2022
    # R version: 4.1.2
    # Purpose: append and prep all individual PULSE 3.4 survey files for analysis
    #
    ...
    #
    # Notes: Phase 3.4 began with week 43 (03/02/2022-03/14/2022) and is scheduled to continue until May 9, 2022.
    #        Like, Phase 3.3, Phase 3.4 has a "two weeks on, two weeks off" sampling scheme.
    #        Phase 3.4 introduced a new question question regarding boosters, new questions regarding education of children
    #        in the last 7 days, split child vaccine data (kiddoses) into two ages groups (5-11 & 12-17). A similar split
    #        in ages was implemented for child vaccination plans (kidgetvac)
    # 
...
## load survey & replicate weights ----
    ...
    setwd(paste0(inputdir, "phase3_4_unzipped/"))
...
## Prepare data ----
    ...
    # create variables ----
        ...
        # Vaccine ----      
          ...
          # KIDDOSES (split for Phase 3.4+)
          dt[, kiddoses_12_17y := factor(kiddoses_12_17y, 
                                  levels = 1:3, 
                                  labels = c("Yes", "No", "Don't know"))]
          
          dt[, kiddoses_5_11y := factor(kiddoses_5_11y, 
                                  levels = 1:3, 
                                  labels = c("Yes", "No", "Don't know"))]
          
          # KIDWHYNO
          for(col in grep("kidwhyno[0-9]", names(dt), value = T)){
            dt[(kidgetvac_5_11y %in% 2:5 | kidgetvac_12_17y %in% 2:5) & is.na(get(col)), paste0(col) := 0]
            dt[, paste0(col) := factor(get(paste0(col)), levels = 0:1, labels = c("No", "Yes"))]
          }   
          
          # KIDGETVAC
          dt[, kidgetvac_5_11y := factor(kidgetvac_5_11y, 
                                   levels = 1:6, 
                                   labels = c("Definitely get the children a vaccine", 
                                              ...
                                              "I do not know the plans for vaccination of the children aged 5-11 in my household"))]
          dt[, kidgetvac_12_17y := factor(kidgetvac_12_17y, 
                                   levels = 1:6, 
                                   labels = c("Definitely get the children a vaccine", 
                                              "Probably get the children a vaccine", 
                                              "Be unsure about getting the children a vaccine", 
                                              "Probably NOT get the children a vaccine", 
                                              "Definitely NOT get the children a vaccine", 
                                              "I do not know the plans for vaccination of the children aged 12-17 in my household"))]
...
## Survey Set for WA & MSA ----
    # Washington ----
        pooledN_svy_wa3_4 <-  # PHASE 3.4 in Washington
          srvyr::as_survey_rep(
        ...
        pooledN_svy_wa3_4 <- dtsurvey::dtrepsurvey(pooledN_svy_wa3_4)
        
        svy_wa3_4 <-  # PHASE 3.4 in Washington (for individual weeks)
          srvyr::as_survey_rep(
            copy(dt[washington==1])  ,
            weights = pweight ,
            combined.weights = TRUE ,
        ...
        svy_wa3_4 <- dtsurvey::dtrepsurvey(svy_wa3_4)
        
    # MSA ----
        pooledN_svy_msa3_4 <-   # PHASE 3.4 in Seattle-Tacoma-Bellevue
          srvyr::as_survey_rep(
            copy(dt[msa==1])  ,
            weights = pooledNwt ,
            combined.weights = TRUE ,
        ...
        pooledN_svy_msa3_4 <- dtsurvey::dtrepsurvey(pooledN_svy_msa3_4)
        
        
        svy_msa3_4 <-   # PHASE 3.4 in Seattle-Tacoma-Bellevue (for individual weeks)
          srvyr::as_survey_rep(
            copy(dt[msa==1])  ,
            weights = pweight ,
            combined.weights = TRUE ,
        ...
        svy_msa3_4 <- dtsurvey::dtrepsurvey(svy_msa3_4)
